...
uint32_t nand_dev_do_cmd(nand_dev_controller_state *s, uint32_t cmd)
{
    ...
    if (cmd == NAND_CMD_WRITE_BATCH || cmd == NAND_CMD_READ_BATCH ||
        ...
        if (goldfish_guest_is_64bit()) {
            cpu_physical_memory_read(bd_addr, (void*)&bd64, sizeof(struct batch_data_64), printData, "nand_Dev_do_cmd_64");
            s->dev = bd64.dev;
            ...
            cpu_physical_memory_read(bd_addr, (void*)&bd, sizeof(struct batch_data), printData, "nand_Dev_do_cmd_else");
            s->dev = bd.dev;
    ...
    case NAND_COMMAND:
        ...
        if (value == NAND_CMD_WRITE_BATCH || value == NAND_CMD_READ_BATCH ||
            ...
            if (goldfish_guest_is_64bit()) {
                ...
                cpu_physical_memory_write(bd_addr, (void*)&bd64, sizeof(struct batch_data_64), printData, "nand_dev_write_nand_64");
            } else {
                bd.result = s->result;
                cpu_physical_memory_write(bd_addr, (void*)&bd, sizeof(struct batch_data), printData, "nand_Dev_write_nand_32");
            }
        }
        break;
    default:
