...
static void
goldfish_audio_buff_read( struct goldfish_audio_buff*  b, int printData, char *text )
{
	//GemDroid added
	...
	cpu_physical_memory_read(b->address, b->data, b->length, printData, text );
}

static void
goldfish_audio_buff_write( struct goldfish_audio_buff*  b, int printData, char *text )
{
	//GemDroid added
    ...
    cpu_physical_memory_write(b->address, b->data, b->length, printData, text);
}
...
static int
goldfish_audio_buff_recv( struct goldfish_audio_buff*  b, int  avail, struct goldfish_audio_state*  s, int printData, char *text )
{
    ...
    cpu_physical_memory_write( b->address + b->offset, b->data, read, printData, text );
    b->offset += read;
...
static uint32_t goldfish_audio_read(void *opaque, hwaddr offset)
{
	...
	if(current_cpu)
	{
		printData = matchMeInPidTid(cpu_single_env);
	}
	...
	case AUDIO_READ_BUFFER_AVAILABLE:
            ...
            goldfish_audio_buff_write( s->in_buff , printData, "goldfish_audio_read");
...
static void goldfish_audio_write(void *opaque, hwaddr offset, uint32_t val)
{
    ...
    switch(offset) {
        ...
        case AUDIO_WRITE_BUFFER_1:
            ...
            goldfish_audio_buff_read( s->out_buff1 , printData, "goldfish_audio_write_1");
        ...
        case AUDIO_WRITE_BUFFER_2:
            ...
            goldfish_audio_buff_read( s->out_buff2, printData, "goldfish_audio_write_2" );
...
goldfish_audio_in_callback(void *opaque, int avail)
{
    ...
    while (avail > 0) {
        int  read = goldfish_audio_buff_recv( s->in_buff, avail, s , printData, "goldfish_audio_buff_recv");
        if (read == 0)
            break;
