...
#define SET_R1_CURRENT_STATE(s)    ((s << 9) & 0x00001E00) /* sx, b (4 bits) */
    ...
    switch (opcode) {
        ...
        case SD_APP_SEND_SCR:
        {
            ...
            scr[1] = 0x00000000;
#endif
            cpu_physical_memory_write(s->buffer_address, (uint8_t*)scr, 8, printData, "goldfish_mmc_do_cmd_SD_APP_SND_SCR");
        ...
        case MMC_SELECT_CARD:
         ...
         case MMC_SWITCH:
            if (arg == 0x00FFFFF1 || arg == 0x80FFFFF1) {
                ...
                buff0[13] = 2;
                cpu_physical_memory_write(s->buffer_address, buff0, sizeof buff0, printData, "goldfish_mmc_do_cmd_MMC_SWITCH");
                new_status |= MMC_STAT_END_OF_DATA;
            }
            s->resp[0] = SET_R1_CURRENT_STATE(4) | R1_READY_FOR_DATA | R1_APP_CMD; //2336
            break;
