...
static void cpu_exec_nocache(CPUArchState *env, int max_cycles, TranslationBlock *orig_tb)
{
	...
	target_disas(0, env, tb->pc,
			tb->size, tb->flags);//ARM_TBFLAG_THUMB(tb->flags));
	addPrintTid(env, 8);
	lastAccessedTemp = getMeContextId(env);
    next_tb = tcg_qemu_tb_exec(env, tb->tc_ptr);
	addPrintTid(env, -8);
	lastAccessedTemp = getMeContextId(env);
	//if(indexPidTid(tb->print_data_flag) >= 0)
	//{
...
static TranslationBlock *tb_find_slow(CPUArchState *env,
                                      ...
                                      uint64_t flags)
{
    //static long notFound_GEMDROID = 0, total_GEMDROID = 0;
    ...
    for(;;) {
        tb = *ptb1;
		//*********************pras********************************************//
		//This is the old code to not found thing...
		//*******************pras**********************************************//
	//if(CPU_tracer && needed_pid != -1)
	//{
	//	#if defined(__arm__)
	//		printf("%lld == arm context id\n", env->cp15.c13_context);
	//		exit(0);
	//	#elif defined(__x86_64__)

	//	//	printf("%lld == x86 context id\n", env-> );
	//	//	exit(0);
	//	#else
	//		printf("not arm, not x86_64\n");
	//		exit(0);
	//	#endif
		//if(needed_pid !=-1)
		//printf("%d %d\n: slow\n", needed_pid, current_pid);
	//if(CPU_tracer && needed_pid == current_pid)
	//{
	//	tb = NULL;
	//	goto not_found;//GEMDROID PRAS - only when the cpu dump collection is on
	//}
        if (!tb)
            goto not_found;
        if (tb->pc == pc &&
 ...
 found:
	...
	//if(!doneLogging && matchMeInPidTid(env))
	//{
	//	//printf("coming inside the new code..");
	//	//target_disas(0, env, tb->pc,
    //    //          tb->size, tb->flags);//ARM_TBFLAG_THUMB(tb->flags));
	//**********************************MOVING ALL PRINTS TO EXEC PLACE!!!!
	//	tb = tb_gen_code(env, pc, cs_base, flags, 0);
	//	//pras
	//}
   // if(CPU_tracer && total_GEMDROID % 10000 == 0)
   // {
   //     printf("GEMDROID NOT FOUND: %lld TOTAL: %lld\n", notFound_GEMDROID, total_GEMDROID);
   // }
	...
	//	if(tb->print_data_flag)
	//	{
	//		extern int indexPidTid(int tid);
	//		if(!indexPidTid(tb->print_data_flag))
	//		{
	//			printf("we have a problem houston!\n");
	//			exit(0);
	//		}
	//	}
	//	tb->print_data_flag = 0;
	//}
    return tb;
}
...
static inline TranslationBlock *tb_find_fast(CPUArchState *env)
{
	...
	//int now_context_id = getMeContextId(env);
	//if(needed_pid == now_context_id)
//	if(matchMeInPidTid(env))
//	{
//		//printf("getting to translate slow after matchign needed pid\n");
...
//	else
	{
		...
		if (unlikely(!tb || tb->pc != pc || tb->cs_base != cs_base ||
					tb->flags != flags)) {
			//printf("getting to translate slow after matchign needed pid %d\n", matchMeInPidTid(env));
			tb = tb_find_slow(env, pc, cs_base, flags);
		}
		tb->print_data_flag = getMeContextId(env);
	}
    ...
    defined(TARGET_MICROBLAZE) || defined(TARGET_LM32) || defined(TARGET_UNICORE32)
...
#endif
#if defined(TARGET_I386)
					printf("cpu exec target i386 defined!!!\n");
					exit(0);
                ...
                if (likely(!cpu->exit_request)) {
					...
					//if(tb->print_data_flag)
					{
						...
						if(getMeContextId(env) != tb->print_data_flag)
						{
							printf("assert failing at 694 line\n");
						}
						assert(getMeContextId(env) == tb->print_data_flag);
					}
					tcg_ctx.print_data_flag = tb->print_data_flag;
