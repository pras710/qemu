                                              ...
                                              uintptr_t retaddr)
{
	...
	{
		printf("@@ %d, io_ld_long(%x,%x,%x=%lu)\n", getMeContextId(env),getMePCVal(env), retaddr, addr, val);
		//printf("@@ %d, io_ld_long(%x,%x,%x,%x=%lu)\n", getMeContextId(env),getMePCVal(env), retaddr,addr, physaddr, val);
		////printf("%s "%s:templs: loading: float %s %s\n", __func__,__FILE__, STRINGIFY(DATA_TYPE), STRINGIFY(SUFFIX));
	}
    return val;
}
                            ...
                            uintptr_t retaddr)
{
    ...
    if (unlikely(tlb_addr & ~TARGET_PAGE_MASK)) {
		...
		if(twoAts && matchMeInPidTid(env))
		{
			printf("@@ %d, io_ld_%s(%x,%x,%x=%lf)\n",getMeContextId(env),STRINGIFY(DATA_TYPE), getMePCVal(env), retaddr, addr, (double)res);
			//printf("%s %s:res:tem gulu pls: loading %s\n", __func__, __FILE__, STRINGIFY(SUFFIX));
		}
        return res;
    }
...
//                     printf("\n HIT");
		...
		{
			//uint32_t phys_addr = cpu_get_phys_page_debug(env, addr);
			//printf("@@ %d, ld_DATA_TYPE%d](%x,%x,%x,%x=%lf)\n",getMeContextId(env), DATA_SIZE, getMePCVal(env), retaddr,addr, phys_addr, (double)res);
			printf("@@ %d, ld_%s[%d](%x,%x,%x=%lf)\n",getMeContextId(env),STRINGIFY(DATA_TYPE),  DATA_SIZE, getMePCVal(env), retaddr+GETPC_ADJ, addr, (double)res);
			//printf("%s %s: res tem gulu pls: loading %s\n", __func__, __FILE__, STRINGIFY(SUFFIX));
		}
        return res;
    }
	...
	{
		//uint32_t phys_addr = cpu_get_phys_page_debug(env, addr);
		printf("@@ %d, ld_%s(%x,%x,%x=%lf)\n", getMeContextId(env), STRINGIFY(DATA_TYPE), getMePCVal(env), retaddr, addr, (double)res);
		////printf("%s "%s: tem  pls: loading: float %s %s\n",  __func__, __FILE__,STRINGIFY(WORD_TYPE), STRINGIFY(LSUFFIX));
	}
    return res;
}
                            ...
                            uintptr_t retaddr)
{
    ...
    if (unlikely(tlb_addr & ~TARGET_PAGE_MASK)) {
		...
		{
			printf("@@ %d, io_ld_%s(%x,%x,%x=%lf)\n",getMeContextId(env), STRINGIFY(DATA_TYPE), getMePCVal(env), retaddr, addr, (double)res);
			//printf("%s %s: tem gulu pls: loading %s \n", __func__, __FILE__, STRINGIFY(SUFFIX));
		}
        return res;
    }
...
//                     printf("\n HIT");
	...
	{
		//uint32_t phys_addr = cpu_get_phys_page_debug(env, addr);
		printf("@@ %d, ld_mmu_%s(%x,%x,%x=%lf)\n", getMeContextId(env), STRINGIFY(DATA_TYPE), getMePCVal(env),GETRA(), addr, (double)res);
	//	printf("%s %s: tem nom  pls: loading: float %s %s\n", __func__, __FILE__, STRINGIFY(WORD_TYPE), STRINGIFY(LSUFFIX));
	}
    return res;
}
#endif /* DATA_SIZE > 1 */
...
glue(glue(helper_ld, SUFFIX), MMUSUFFIX)(CPUArchState *env, target_ulong addr,
                                         int mmu_idx)
{
	DATA_TYPE res = helper_te_ld_name (env, addr, mmu_idx, GETRA()) ;
	if(twoAts && matchMeInPidTid(env))
	{
		//printf("%s "%s\n",__FUNCTION__,__FILE__);
		printf("@@ %d, ld_mmu_%s(%x,%x,%x=%lf)\n", getMeContextId(env), STRINGIFY(DATA_TYPE), getMePCVal(env),GETRA(), addr, (double)res);
	...
	//{
	//	//printf("590 %s %s: tem gulu pls: loading %s %s %s\n",  __func__, __FILE__, STRINGIFY(WORD_TYPE), STRINGIFY(MMUSUFFIX), STRINGIFY(SUFFIX));
	//}
    return res; 
}
                             ...
                             int mmu_idx, uintptr_t retaddr)
{
	//if(matchMeInPidTid(env))
	//{
	//	//printf("605 %s %s:tem gulu pls: loading %s\n", __func__, __FILE__, STRINGIFY(WORD_TYPE));
	//}
	//printf("%s "%s\n",__FUNCTION__,__FILE__);
    SDATA_TYPE res = (SDATA_TYPE)helper_le_ld_name(env, addr, mmu_idx, retaddr);
	if(twoAts && matchMeInPidTid(env))
	{
		printf("@@ %d, lds_le_%s(%x,%x,%x=%lf)\n", getMeContextId(env), STRINGIFY(SDATA_TYPE), getMePCVal(env),retaddr, addr, (double)res);
	//	printf("%s %s: tem nom  pls: loading: float %s %s\n", __func__, __FILE__, STRINGIFY(WORD_TYPE), STRINGIFY(LSUFFIX));
	}
	return res;
}
                             ...
                             int mmu_idx, uintptr_t retaddr)
{
	//if(matchMeInPidTid(env))
	//{
	//	//printf("616 %s %s: tem gulu pls: loading  %s\n",  __func__, __FILE__, STRINGIFY(WORD_TYPE));
	//}
	//printf("%s "%s\n",__FUNCTION__,__FILE__);
    SDATA_TYPE res = (SDATA_TYPE)helper_be_ld_name(env, addr, mmu_idx, retaddr);
	if(twoAts && matchMeInPidTid(env))
	{
		printf("@@ %d, lds_be_%s(%x,%x,%x=%lf)\n", getMeContextId(env), STRINGIFY(SDATA_TYPE), getMePCVal(env),retaddr, addr, (double)res);
	}
	return res;
}
# endif
                                          ...
                                          uintptr_t retaddr)
{
	...
	if(twoAts && matchMeInPidTid(env))
	{
		printf("@@ %d, io_st_%s(%x,%x,%x=%lf)\n", getMeContextId(env), STRINGIFY(DATA_TYPE), getMePCVal(env),retaddr, addr, (double)val);
		////printf("%s "%s: tem gulu pls: loading  %s\n",  __func__, __FILE__, STRINGIFY(SUFFIX));
	}
}
                       ...
                       int mmu_idx, uintptr_t retaddr)
{
    ...
    do_unaligned_access:
		...
		{
			//uint32_t phys_addr = cpu_get_phys_page_debug(env, addr);
			printf("@@ %d, st_%s[%d](%x,%x,%x=%lf)\n",getMeContextId(env), STRINGIFY(DATA_TYPE), DATA_SIZE,getMePCVal(env),retaddr, addr, (double)val);
			//printf(" %s %s: tem gulu pls: loading %s\n",  __func__, __FILE__, STRINGIFY(SUFFIX));
		}
        return;
    }
                       ...
                       int mmu_idx, uintptr_t retaddr)
{
    ...
    do_unaligned_access:
		...
		if(twoAts && matchMeInPidTid(env))
		{
			////printf("%s "%s: tem gulu pls: loading %s \n",__func__, __FILE__,  STRINGIFY(SUFFIX));
			//uint32_t phys_addr = cpu_get_phys_page_debug(env, addr);
			printf("@@ %d, st_%s[%d](%x,%x,%x=%lf)\n",getMeContextId(env), STRINGIFY(DATA_TYPE), DATA_SIZE, getMePCVal(env),retaddr+GETPC_ADJ, addr, (double)val);
		}
        return;
    }
                                         ...
                                         DATA_TYPE val, int mmu_idx)
{
	...
	if(twoAts && matchMeInPidTid(env))
	{
		printf("@@ %d, st_%s(%x,%x,%x=%lf)\n",getMeContextId(env), STRINGIFY(DATA_TYPE) STRINGIFY(SUFFIX) STRINGIFY(MMUSUFFIX), getMePCVal(env),GETRA(), addr, (double)val);
		//printf("1057 %s %s: tem gulu pls: loading %s\n", __func__, __FILE__, STRINGIFY(SUFFIX));
	}
