...
//it could save some exec time	if(matchMeInPidTid(env))
//it could save some exec time	{
//it could save some exec time		//pras
//it could save some exec time		printf("@@@ %d set cpsr=%u\n", getMeContextId(env), cpsr_read(env), val);
//it could save some exec time	}
}
...
//it could save some exec time	{
//it could save some exec time		//pras
//it could save some exec time		printf("@@@ %d get r%d=%u\n", getMeContextId(env), regno, val);
//it could save some exec time	}
    return val;
}
...
//it could save some exec time	{
//it could save some exec time		//pras
//it could save some exec time		printf("@@@ %d set r%d=%u\n", getMeContextId(env), regno, val);
//it could save some exec time	}
    if (regno == 13) {
...
#define STB(addr, val) if(matchMeInPidTid(env))printf("#$#$ %d, %x, st %x=%x\n", getMeContextId(env), getMePCVal(env), addr, val);helper_stb_mmu(env, addr, val, user)
#define STW(addr, val) if(matchMeInPidTid(env))printf("#$#$ %d, %x, st %x=%x\n", getMeContextId(env), getMePCVal(env), addr, val);helper_le_stw_mmu(env, addr, val, user, GETPC())
#define STL(addr, val) if(matchMeInPidTid(env))printf("#$#$ %d, %x, st %x=%x\n", getMeContextId(env), getMePCVal(env), addr, val);helper_le_stl_mmu(env, addr, val, user, GETPC())
#define STQ(addr, val) if(matchMeInPidTid(env))printf("#$#$ %d, %x, st %x=%x\n", getMeContextId(env), getMePCVal(env), addr, val);helper_le_stq_mmu(env, addr, val, user, GETPC())
#endif
    static const struct {
        int nregs;
        int interleave;
    ...
    int i, reg;
    uint64_t tmp64;
	int printFlag = matchMeInPidTid(env), tid = getMeContextId(env), pc = getMePCVal(env);
        ...
        switch (size) {
            case 3:
                if (load) {
					...
					if(printFlag)
					{
						printf("@@@ %d, vfp_ld(%x=%lf)\n", tid, pc, env->vfp.regs[rd]);
					}
                } else {
                    STQ(addr, float64_val(env->vfp.regs[rd]));
					if(printFlag)
					{
						printf("@@@ %d, vfp_st(%x=%lf)\n", tid, pc, env->vfp.regs[rd]);
					}
                }
                addr += stride;
                break;
