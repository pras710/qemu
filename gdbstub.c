...
static int gdb_handle_packet(GDBState *s, const char *line_buf)
{
    ...
    case 'm':
        ...
        if (cpu_memory_rw_debug(ENV_GET_CPU(s->g_cpu), addr, mem_buf, len, 0, "gdbstub") != 0) {
            put_packet (s, "E14");
        } else {
            memtohex(buf, mem_buf, len);
            put_packet(s, buf);
    ...
    case 'M':
        ...
        if (cpu_memory_rw_debug(ENV_GET_CPU(s->g_cpu), addr, mem_buf, len, 1, "gdbstub") != 0)
            put_packet(s, "E14");
        else
            put_packet(s, "OK");
        break;
