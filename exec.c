...
void cpu_physical_memory_rw(hwaddr addr, void *buf,
                            int len, int is_write, int printData, char *text)
{
    int l, io_index;
    uint8_t *ptr;
    uint32_t val;
...
//	if(current_cpu && matchMeInPidTid(cpu_single_env) && strcmp(text, "disas"))
//	{
//		printf("@@@@ %d, cpu_rom_phy_write[%d]%s %x=%x, print?%d\n", getMeContextId(cpu_single_env),len, text, getMePCVal(cpu_single_env), *buf8, printData);
//	}
	//else
	//{
	//	printf("@@@@ cpu_null_phy_rw %s\n",  text);
	//}
    while (len > 0) {
        page = addr & TARGET_PAGE_MASK;
        l = (page + TARGET_PAGE_SIZE) - addr;
...
//				if(printData)
//				{
//					printf("@@@@ %d, cpu_rom_phy_write[%d]%s %x=%x\n", getMeContextId(cpu_single_env),l, text, getMePCVal(cpu_single_env), *buf8);
//				}
            } else {
                ram_addr_t addr1;
...
//				if(printData)
//				{
//					printf("@@@@ %d, cpu_phy_write[%d]%s %x=%x\n", getMeContextId(cpu_single_env),l, text, getMePCVal(cpu_single_env), *buf8);
//				}
...
//				if(printData)
//				{
//					printf("@@@@ %d, cpu_rom_phy_read[%d]%s %x=%x\n", getMeContextId(cpu_single_env),l,text, getMePCVal(cpu_single_env), *buf8);
//				}
            } else {
                /* RAM case */
                              ...
                              int is_write)
{
    ...
    while (len > 0) {
        ...
        if ((pd & ~TARGET_PAGE_MASK) != IO_MEM_RAM) {
            ...
            bounce.len = l;
            if (!is_write) {
                cpu_physical_memory_read(addr, bounce.buffer, l, printData, "physical_memory_map");
            }
            ptr = bounce.buffer;
        } else {
            addr1 = (pd & TARGET_PAGE_MASK) + (addr & ~TARGET_PAGE_MASK);
                               ...
                               int is_write, hwaddr access_len)
{
    ...
    if (is_write) {
		...
		int printData = matchMeInPidTid(cpu_single_env);
        cpu_physical_memory_write(bounce.addr, bounce.buffer, access_len, printData, "phy_mem_unmap");
    }
...
//			if(printData)
//			{
//				printf("@@@ %d, ldl_internal %x=%lf\n", getMeContextId(cpu_single_env), 
//						getMePCVal(cpu_single_env), (double)val);
//			}
//		}
    }
    return val;
...
uint32_t ldub_phys(hwaddr addr)
{
    uint8_t val;
	int printData = matchMeInPidTid(cpu_single_env);
    cpu_physical_memory_read(addr, &val, 1, printData, "ldub_phys");
    return val;
}
...
void stl_phys_notdirty(hwaddr addr, uint32_t val)
{
	...
	if(current_cpu)
	{
		int printData = matchMeInPidTid(cpu_single_env);//((CPUArchState*)tls__current_cpu->env_ptr));//
		if(printData)
		{
			printf("@@@ %d, stl_internal %x=%lf\n", getMeContextId(cpu_single_env), 
					getMePCVal(cpu_single_env), (double)val);
		}
	}
}
...
void stb_phys(hwaddr addr, uint32_t val)
{
    uint8_t v = val;
	int printData = matchMeInPidTid(cpu_single_env);
    cpu_physical_memory_write(addr, &v, 1, printData, "stb_phys");
}
...
void stq_phys(hwaddr addr, uint64_t val)
{
    val = tswap64(val);
	int printData = matchMeInPidTid(cpu_single_env);
    cpu_physical_memory_write(addr, &val, 8, printData, "stq_pyhs");
}
...
void stq_le_phys(hwaddr addr, uint64_t val)
{
    val = cpu_to_le64(val);
	int printData = matchMeInPidTid(cpu_single_env);
    cpu_physical_memory_write(addr, &val, 8, printData, "stq_le_phys");
}
...
void stq_be_phys(hwaddr addr, uint64_t val)
{
    val = cpu_to_be64(val);
	int printData = matchMeInPidTid(cpu_single_env);
    cpu_physical_memory_write(addr, &val, 8, printData, "stq_be_phys");
}
                        ...
                        void *buf, int len, int is_write, char *text)
{
	...
	int printData = matchMeInPidTid(env) && strcmp(text, "disas");
    while (len > 0) {
        page = addr & TARGET_PAGE_MASK;
...
#endif
            cpu_physical_memory_rw(phys_addr, buf8, l, is_write, printData, text);
        len -= l;
        buf8 += l;
        addr += l;
    }
