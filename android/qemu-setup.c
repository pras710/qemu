...
{
	printf("this is getting called %d %s %s\n", __LINE__, __FILE__, __func__);
    ...
    if (android_op_ports) {
        ...
        for ( ; tries > 0; tries--, base_port += 2 ) {
            ...
            if (legacy_adb) {
				...
				{
					printf("legacy slirp redir worked\n");
					continue;
				}
				printf("did not continue, legacy adb = %d\n", legacy_adb);
            } else {
			...
			int console_started = control_console_start(base_port);
			printf("console start = %d ( < 0)?, base port = %d\n", console_started, base_port);
            /* setup second redirection for the emulator console */
            if ( console_started < 0 ) {
                if (legacy_adb) {
					printf("legacy slirp unredir\n");
                    slirp_unredir( 0, adb_port );
                }
                continue;
            }
    ...
    */
    ...
    if (s < 0) {
        ...
        int len = 0;//snprintf(tmp, sizeof tmp, "0000host:emulator:%d", adb_port);
		//pras did this:
		//   char tmp_pras[32], header_pras[5];
		//   len = snprintf(tmp_pras, sizeof tmp_pras, "shell:ls /");
		//   socket_send(s, tmp_pras, len);
		//   android_adb_service_init();
        //   D("sent '%s' to ADB server", tmp_pras);
        //   socket_close(s);
		//pras end
		s = socket_loopback_client(adb_host_port, SOCKET_STREAM);
        len = snprintf(tmp, sizeof tmp, "0000host:emulator:%d", adb_port);
...
#  define  _ANDROID_PING_PROGRAM   "ddms"
        ...
        if (path_exists(tmp)) {
            ...
            printf( "ping command: %s %s", comspec, args );
#else
            int  pid;
