...
int main(int argc, char **argv)
{
     ...
     */
    {
        ...
        if (opts->shell || opts->logcat) {
			//p = bufprint(p, end, "androidboot.console=pipe:pras_console_dumper");
            p = bufprint(p, end, " androidboot.console=%s%d",
                         androidHwConfig_getKernelSerialPrefix(android_hw),
                         shell_serial );
			//printf(" androidboot.console=%s%d",
            //             androidHwConfig_getKernelSerialPrefix(android_hw),
            //             shell_serial );
			//printf("\n");
			//exit(0);
        ...
        if (opts->logcat) {
            char*  q = bufprint(p, end, " androidboot.logcat=thread");//, opts->logcat);
